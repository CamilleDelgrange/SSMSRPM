...
* Based on MMCL codebase https://github.com/paulhager/MMCL-Tabular-Imaging/blob/main/models/TabularEmbeddingModel.py
'''
...
class TabularEmbeddingModel(nn.Module):
  ...
  def __init__(self, args):
    ...
    for k in list(state_dict.keys()):
      if k.startswith('encoder_tabular.encoder.'):
        state_dict_encoder[k[len('encoder_tabular.encoder.'):]] = state_dict[k]
      if k.startswith('projector_tabular.layers.'):
        state_dict_projector[k[len('projector_tabular.layers.'):]] = state_dict[k]
  ...
  def build_encoder(self, original_args: Dict) -> nn.Sequential:
    ...
    for _ in range(original_args['encoder_num_layers']-1):
      modules.extend([nn.BatchNorm1d(original_args['embedding_dim']), nn.ReLU(), nn.Dropout(p=original_args['dropout_rate']), nn.Linear(original_args['embedding_dim'], original_args['embedding_dim'])])
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    print("Outputted embeddings", embeddings.shape)
