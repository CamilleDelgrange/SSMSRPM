...
class CrossAttention(nn.Module): #num_heads = 4
    def __init__(self, q_dim, k_dim, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., with_qkv=True):
...
class Block(nn.Module):
    def __init__(self, dim, num_heads=4, is_cross_attention=False, encoder_dim=None, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
...
class TabularTransformerEncoder(nn.Module):
    ...
    def embedding(self, x, mask_special=None):
        ...
        #print("dim x cat & con concat", x.shape)
        # mask special token
        ...
        #print("concat cls tokens and x ", x.shape)
        #print("pos_ids", self.pos_ids)
...
class MultimodalTransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, image_features: torch.Tensor, visualize=False) -> torch.Tensor:
        #print("Entering transformer", image_features.shape)
        if len(image_features.shape) == 5:
            B,C,H,W,D = image_features.shape # 5 dimensions outputted from 3D ResNet encoder.
            image_features = image_features.reshape(B,C,H*W*D).permute(0,2,1)
        if len(image_features.shape) == 2:
            image_features = image_features.unsqueeze(1)
        #print("Entering projector", image_features.shape)
        image_features = self.image_proj(image_features)
        image_features = self.image_norm(image_features)
        #print("image features", image_features.shape) 
        #print("Entering projector", x.shape)           
        x = self.tabular_proj(x)
        #print("Tab projected", x.shape)
        if len(x.shape) == 2: 
            x = x.unsqueeze(1) # for transformer token
        #print("Tab size", x.shape)
        if visualize == False:
...
if __name__ == "__main__":
    ...
    # embedding dim: before the features are projected to the multimodal embedding dimension. 
    ...
    #print(out.shape) # [2, 6, 512] = batch_size, seq length after processing i.e. # of orig features + additional token special
    # [CLS] token or padding, embedding dimension matching multimodal_embedding_dim.   

    # ### Multimodal Transformer Encoder Test
    x = torch.randn(2, 6, 512) #output from the tabular transformer encoder: batch_size, seq length, embedding_dim
    image_features = torch.randn(2, 64, 2048) #batch_size, 64: Sequence length, representing the number of tokens/features extracted from the image (for example, this could be from a 7x7 grid of features from a ResNet or ViT).
    # 2048: Embedding dimension (size of the feature vectors for each of the 49 spatial positions).
