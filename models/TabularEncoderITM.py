...
class TabularEncoder(nn.Module):
  ...
  def __init__(self, args) -> None:
    ...
    if args.checkpoint:
      if args.checkpoint_tabular or args.checkpoint_multimodal:
        ...
        #print(original_args['tabular_embedding_dim'])
        #print(original_args['encoder_num_layers'])
  ...
  def build_encoder(self, args: Dict) -> nn.Sequential:
    modules = [nn.Linear(self.input_size, args['tabular_embedding_dim'])] #embedding_dim
    for _ in range(args['encoder_num_layers']-1):
      modules.extend([nn.BatchNorm1d(args['tabular_embedding_dim']), nn.ReLU(), nn.Dropout(p=args['dropout_rate']), nn.Linear(args['tabular_embedding_dim'], args['tabular_embedding_dim'])])
    return nn.Sequential(*modules)
  
  def build_encoder_no_bn(self, args: Dict) -> nn.Sequential:
    modules = [nn.Linear(self.input_size, args['tabular_embedding_dim'])]
    for _ in range(args['encoder_num_layers']-1):
      modules.extend([nn.ReLU(), nn.Dropout(p=args['dropout_rate']), nn.Linear(args['tabular_embedding_dim'], args['tabular_embedding_dim'])])
    return nn.Sequential(*modules)

  def build_encoder_bn_old(self, args: Dict) -> nn.Sequential:
    modules = [nn.Linear(args.input_size, args.tabular_embedding_dim)]
    for _ in range(args.encoder_num_layers-1):
      modules.extend([nn.ReLU(), nn.BatchNorm1d(args.tabular_embedding_dim), nn.Dropout(p=args['dropout_rate']), nn.Linear(args.tabular_embedding_dim, args.tabular_embedding_dim)])
    return nn.Sequential(*modules)
