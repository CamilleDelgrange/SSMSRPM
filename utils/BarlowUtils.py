...
class BarlowTwinsLoss(nn.Module):
    ...
    def off_diagonal_ele(self, x):
        ...
        # Ensures x is a torch.Tensor (convert from MetaTensor if necessary)
        # print(x, x.shape)
        # Convert MetaTensor to plain torch.Tensor
        if isinstance(x, torch.Tensor):
            if hasattr(x, 'as_tensor'):  # Check if it's a MetaTensor
                x = x.as_tensor()  # Convert MetaTensor to torch.Tensor
            else:
                x = x.detach()  # Detach if it's a regular Tensor
        n, m = x.shape
        assert n == m
    ...
    def forward(self, z1, z2):
        ...
        # Print the magnitude of z1 and z2
        print(f"Magnitude of z1: {torch.norm(z1, dim=1).mean().item()}")
        print(f"Magnitude of z2: {torch.norm(z2, dim=1).mean().item()}")

        # Ensure cross_corr is a torch.Tensor (convert if necessary)
        if hasattr(cross_corr, 'as_tensor'):  # Check if it's a MetaTensor
            cross_corr = cross_corr.as_tensor()
...
class ProjectionHead(nn.Module):
    def __init__(self, num_layer=3, input_dim=512, hidden_dim=1024, output_dim=2048, last_bn=True):
        super().__init__()
    ...
    def _initialize_weights(self):
        for layer in self.projection_head:
            if isinstance(layer, nn.Linear):
                nn.init.kaiming_normal_(layer.weight)  # Kaiming initialization for linear layers
